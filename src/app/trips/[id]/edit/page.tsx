...
export default function ItineraryBuilder() {
  ...
  // Mock City Database
  const CITY_DB = [
    { name: "Tokyo", country: "Japan", costIndex: 4, popularity: 98, image: "https://images.unsplash.com/photo-1542051841857-5f90071e7989?auto=format&fit=crop&q=80&w=400" },
  ...
  const handleDownloadItinerary = () => {
    ...
    if (!printWindow) {
      alert("Please allow popups to download the itinerary.");
      return;
    }
    ...
    let html = `
      ...
      <html>
        <head>
          <title>Traveloop Itinerary - ${trip.name}</title>
          <link rel="preconnect" href="https://fonts.googleapis.com">
          <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
          <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;700;900&display=swap" rel="stylesheet">
          <style>
            body { font-family: 'Inter', -apple-system, sans-serif; padding: 40px; color: #1a1a1a; line-height: 1.6; max-width: 800px; margin: 0 auto; background: #fff; }
            .header { border-bottom: 4px solid #00c9ff; padding-bottom: 20px; margin-bottom: 40px; }
            h1 { font-weight: 900; font-size: 38px; margin: 0; color: #000; letter-spacing: -0.02em; }
            .meta { color: #666; font-size: 14px; margin-top: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.1em; }
            .day { margin-bottom: 50px; page-break-inside: avoid; }
            .day-title { font-size: 24px; font-weight: 900; background: #f8f8f8; padding: 12px 20px; border-radius: 12px; margin-bottom: 25px; display: flex; justify-content: space-between; align-items: center; }
            .activity { margin-bottom: 20px; padding-left: 20px; border-left: 3px solid #00c9ff; position: relative; }
            .activity-header { display: flex; align-items: baseline; gap: 10px; margin-bottom: 4px; }
            .activity-time { font-weight: 900; color: #00c9ff; font-size: 14px; min-width: 70px; }
            .activity-name { font-weight: 800; font-size: 18px; color: #000; }
            .activity-desc { color: #4a4a4a; font-size: 14px; margin-top: 4px; }
            .activity-tags { font-size: 11px; color: #888; font-weight: 700; text-transform: uppercase; margin-top: 6px; letter-spacing: 0.05em; }
            .footer { margin-top: 80px; text-align: center; border-top: 1px solid #eee; padding-top: 30px; color: #aaa; font-size: 12px; font-weight: 600; }
            @media print {
              body { padding: 20px; }
              @page { margin: 2cm; }
            }
    ...
    trip.stops?.forEach((stop: any, index: number) => {
      html += `
        <div class="day">
          <div class="day-title">
            <span>Day ${index + 1}: ${stop.cityName}</span>
            <span style="font-size: 14px; color: #888;">${new Date(stop.arrivalDate).toLocaleDateString('en-US', { weekday: 'long' })}</span>
          </div>
      `;

      const sortedActivities = [...(stop.activities || [])].sort((a: any, b: any) => 
        (a.timeStart || '').localeCompare(b.timeStart || '')
      );

      sortedActivities.forEach((activity: any) => {
        html += `
          <div class="activity">
            <div class="activity-header">
              <span class="activity-time">${activity.timeStart || 'Day'}</span>
              <span class="activity-name">${activity.name}</span>
            </div>
            <div class="activity-desc">${activity.description || ''}</div>
            <div class="activity-tags">
              ${activity.category} • ${activity.duration} Mins • Est. $${activity.cost}
            </div>
          </div>
        `;
      });
    ...
    html += `
          <div class="footer">
            GENERATED BY TRAVELOOP AI | YOUR JOURNEY, OUR INTELLIGENCE.
          </div>
          <script>
